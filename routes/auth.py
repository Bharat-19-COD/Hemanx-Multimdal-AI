    ...
    if request.method == 'POST':
        ...
        if user:
            ...
            if user.role == 'admin':
                ...
                return redirect(next_page or url_for('dashboard.student'))
        else:
            flash('Invalid email or password', 'error')
...
def register():
    ...
    if request.method == 'POST':
        ...
        user, message = User.create_user(db, email, password, name, role)
        if user:
            flash('Registration successful! Please login.', 'success')
            return redirect(url_for('auth.login'))
        else:
            flash(message or 'Registration failed', 'error')
